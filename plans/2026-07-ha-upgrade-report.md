...
- ✅ **Blocker 1 DONE** — all 73 legacy template files migrated to modern `template:` syntax (77 entities). Entity IDs preserved via `default_entity_id` + `unique_id`. Merge-dir files moved to new `template/` dir (`template: !include_dir_merge_list template/` added to configuration.yaml). Validated with `hass --script check_config` under both 2026.4.3 and 2026.7.4: zero template errors, findings identical to pre-migration baseline. NOTE: on 2026.7.4 the legacy syntax does NOT hard-fail — it silently creates zero entities + a Repairs issue, so this migration prevented a silent wipe of all 77 entities. Awaiting commit + deploy.
